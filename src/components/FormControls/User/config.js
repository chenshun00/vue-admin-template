...
import { GET_ALL_USER, GET_DEPT_ROOT, GET_DEPT_TREE, GET_PAGE_EMPLOYEE } from '@/api'
...
async function loadDepOrUser(node, loadDep = true) {
  ...
  if (node.level === 0) { // 根目录
    ...
    nodeData = (await GET_PAGE_EMPLOYEE()).data.data
  }
  return nodeData
}
...
async function getRootDept() {
  let res = []
  try {
    res = (await GET_DEPT_ROOT()).data.data
  } catch (err) {
    console.error(err)
  }
  return res
